...
module.exports = function (primaryModel, associatedModel, hostConfig) {
  ...
  return _ref6 = {}, _defineProperty(_ref6, 'find' + primaryModelNameCap + pluralAssociatedModelNameCap, function (primaryModelId, associatedModelId) {
    ...
    return function (dispatch) {
      ...
      return axios.get(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + associatedModelId).then(function (res) {
        return dispatch(success(res.data.data));
      }).catch(function (res) {
        return dispatch(error(res.data, primaryModelId));
      });
    };
  }), _defineProperty(_ref6, 'add' + singleAssociatedModelNameCap + 'To' + primaryModelNameCap, function (primaryModelId, modelToAssociate, alreadyAssociatedModels) {
    ...
    if ('id' in modelToAssociate) {
      return function (dispatch) {
        ...
        return axios.post(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + '/' + modelToAssociate.id).then(function (res) {
      ...
      // Means that model does not exists in database
        ...
        return function (dispatch) {
          ...
          return axios.post(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel, modelToAssociate).then(function (res) {
            return dispatch(success(associatedModelWithTmpId));
          }).catch(function (res) {
            return dispatch(error(res.data, associatedModelWithTmpId));
          });
  ...
  }), _defineProperty(_ref6, 'remove' + singleAssociatedModelNameCap + 'From' + primaryModelNameCap, function (primaryModelId, modelToDissociate) {
    ...
    return function (dispatch) {
      ...
      return axios.delete(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + '/' + modelToDissociate.id).then(function (res) {
