...
module.exports = function (primaryModel, associatedModel, hostConfig) {
  ...
  return _ref6 = {}, _defineProperty(_ref6, findPrimaryAssociatedModels, function (primaryModelId, associatedModelId) {
    ...
    return function (dispatch) {
      ...
      return providerUtil(hostConfig, 'get', baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + associatedModelId, headers[findPrimaryAssociatedModels]).then(function (res) {
        return dispatch(success(res));
      }).catch(function (res) {
        return dispatch(error(res, primaryModelId));
      });
    };
    ...
    var url = 'id' in modelToAssociate ? baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + '/' + modelToAssociate.id : baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel;

    return function (dispatch) {
      var associatedModelWithTmpId = dispatch(start(modelToAssociate))[primaryModelName + singleAssociatedModelNameCap];

      return providerUtil(hostConfig, 'post', url, headers[addAssociatedModelToPrimary], modelToAssociate).then(function (res) {
        return dispatch(success(associatedModelWithTmpId));
      }).catch(function (res) {
        return dispatch(error(res, associatedModelWithTmpId));
      });
    };
    ...
    return function (dispatch) {
      ...
      return providerUtil(hostConfig, 'delete', baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + '/' + modelToDissociate.id, headers[removeAssociatedModelFromPrimary]).then(function (res) {
        return dispatch(success(modelToDissociate));
      }).catch(function (res) {
        return dispatch(error(res, modelToDissociate));
      });
    };
  }), _ref6;
};