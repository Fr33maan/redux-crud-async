  ...
  return _ref6 = {}, _defineProperty(_ref6, findPrimaryAssociatedModels, function (primaryModelId, associatedModelId) {
    ...
    if ('id' in modelToAssociate) {
      return function (dispatch) {
        ...
        return providerUtil(hostConfig, 'post', baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + '/' + modelToAssociate.id, headers[addAssociatedModelToPrimary], modelToAssociate).then(function (res) {
      ...
      // Means that model does not exists in database
        ...
        return function (dispatch) {
          ...
          return providerUtil(hostConfig, 'post', baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel, headers[addAssociatedModelToPrimary], modelToAssociate).then(function (res) {
            return dispatch(success(associatedModelWithTmpId));
          }).catch(function (res) {
            return dispatch(error(res.data, associatedModelWithTmpId));
          });
