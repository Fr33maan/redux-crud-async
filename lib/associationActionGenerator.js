...
module.exports = function (primaryModel, associatedModel, hostConfig) {
  ...
  if (authConfig && hasSessionStorage) {
    (function () {
      ...
      var JWT_Token = sessionStorage.getItem(sessionStorageName);
  ...
  return _ref6 = {}, _defineProperty(_ref6, findPrimaryAssociatedModels, function (primaryModelId, associatedModelId) {
    ...
    return function (dispatch) {
      ...
      return axios.get(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + associatedModelId, bearers[findPrimaryAssociatedModels]).then(function (res) {
    ...
    if ('id' in modelToAssociate) {
      return function (dispatch) {
        ...
        return axios.post(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + '/' + modelToAssociate.id, bearers[addAssociatedModelToPrimary]).then(function (res) {
      ...
      // Means that model does not exists in database
        ...
        return function (dispatch) {
          ...
          return axios.post(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel, modelToAssociate, bearers[addAssociatedModelToPrimary]).then(function (res) {
            return dispatch(success(associatedModelWithTmpId));
          }).catch(function (res) {
            return dispatch(error(res.data, associatedModelWithTmpId));
          });
    ...
    return function (dispatch) {
      ...
      return axios.delete(baseUrl + '/' + urlPrimaryModel + '/' + primaryModelId + '/' + urlAssociatedModel + '/' + modelToDissociate.id, bearers[removeAssociatedModelFromPrimary]).then(function (res) {
